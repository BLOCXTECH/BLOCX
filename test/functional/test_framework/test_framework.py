...
TMPDIR_PREFIX = "blocx_func_test_"
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave blocxds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop blocxds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use blocx-cli instead of RPC for all commands")
        parser.add_argument("--blocxd-arg", dest="blocxd_extra_args", default=[], action="append",
                            help="Pass extra args to all blocxd instances")
    ...
    def setup(self):
        ...
        self.options.bitcoind = os.getenv("BITCOIND", default=config["environment"]["BUILDDIR"] + '/src/blocxd' + config["environment"]["EXEEXT"])
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=config["environment"]["BUILDDIR"] + '/src/blocx-cli' + config["environment"]["EXEEXT"])

        self.extra_args_from_options = self.options.blocxd_extra_args
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: blocxds were not stopped and may still be running")
    ...
    def dynamically_initialize_datadir(self, chain, node_p2p_port, node_rpc_port):
        ...
        if chain == 'testnet3':
            ...
            chain_name_conf_arg_value = '1'
        with open(os.path.join(data_dir, "blocx.conf"), 'w', encoding='utf8') as f:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as blocxd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain)  # Overwrite port/rpcport in blocx.conf
    ...
    def skip_if_no_bitcoind_zmq(self):
        """Skip the running test if blocxd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("blocxd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if blocx-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("blocx-wallet has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if blocx-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("blocx-cli has not been compiled.")

    def is_cli_compiled(self):
        """Checks whether blocx-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether blocx-wallet was compiled."""
...
class BLOCXTestFramework(BitcoinTestFramework):
    def set_test_params(self):
    ...
    def set_blocx_test_params(self, num_nodes, masterodes_count, extra_args=None, fast_dip3_enforcement=False, hpmn_count=0):
        ...
        self.set_blocx_dip8_activation(200)

        # LLMQ default test params (no need to pass -llmqtestparams)
        self.llmq_size = 3
        self.llmq_threshold = 2

        # This is nRequestTimeout in blocx-q-recovery thread
    ...
    def set_blocx_dip8_activation(self, activate_after_block):
    ...
    def set_blocx_llmq_test_params(self, llmq_size, llmq_threshold):
