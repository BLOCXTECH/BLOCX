...
def setup():
    ...
    if not os.path.isdir('blocx-detached-sigs'):
        subprocess.check_call(['git', 'clone', 'https://github.com/blocxpay/blocx-detached-sigs.git'])
...
def build():
    ...
    subprocess.check_call(['make', '-C', '../blocx/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def main():
    ...
    if args.pull:
        ...
        os.chdir('../gitian-builder/inputs/blocx')
        subprocess.check_call(['git', 'fetch', args.url, 'refs/pull/'+args.version+'/merge'])
