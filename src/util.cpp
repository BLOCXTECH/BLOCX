...
const char * const BITCOIN_PID_FILENAME = "blocxd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "blocxd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BLOCX
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BLOCX
    // Mac: ~/Library/Application Support/BLOCX
    // Unix: ~/.blocx
#ifdef WIN32
    // Windows
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty blocx.conf if it does not excist
