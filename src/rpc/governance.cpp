...
static UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 BLOCX irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on blocxcentral etc. and do not save them...
...
static void gobject_vote_conf_help(const JSONRPCRequest& request)
{
    RPCHelpMan{"gobject vote-conf",
        "Vote on a governance object by masternode configured in blocx.conf\n",
        {
...
static UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.pushKV("blocx.conf", statusObj);
...
#endif // ENABLE_WALLET
        "  vote-conf          - Vote on a governance object by masternode configured in blocx.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    /* BLOCX features */
    { "blocx",               "getgovernanceinfo",      &getgovernanceinfo,      {} },
    { "blocx",               "getsuperblockbudget",    &getsuperblockbudget,    {"index"} },
    { "blocx",               "gobject",                &gobject,                {} },
    { "blocx",               "voteraw",                &voteraw,                {"tx_hash","tx_index","gov_hash","signal","outcome","time","sig"} },
