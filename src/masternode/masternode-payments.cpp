...
*
*   Why is this needed?
*   - In BLOCX some blocks are superblocks, which output much higher amounts of coins
...
bool CMasternodePayments::GetBlockTxOuts(int nBlockHeight, CAmount blockReward, std::vector<CTxOut>& voutMasternodePaymentsRet)
{
    ...
    bool isMnTierForkActivated = consensusParams.MNTierForkHeight <= pindex->nHeight;
    ...
    if (isMnTierForkActivated) {
        ...
        if (dmnPayee2) {
            mnTierOneReward = (masternodeReward * consensusParams.RegularNodePercentage) / 100;
