...
impl PrivateKey {
    ...
    #[cfg(feature = "blocx_helpers")]
    pub fn generate_blocx(rng: &mut StdRng) -> Result<Self, BlsError> {
    ...
    #[cfg(feature = "blocx_helpers")]
    pub fn generate_blocx_many(count: usize, rng: &mut StdRng) -> Result<Vec<Self>, BlsError> {
        (0..count)
            .into_iter()
            .map(|_| Self::generate_blocx(rng))
            .collect()
    }
