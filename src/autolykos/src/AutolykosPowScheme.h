...
public:
    ...
    boost::multiprecision::cpp_int genElement(const std::vector<uint8_t>& indexBytes,
                                          const std::vector<uint8_t>& heightBytes);

    std::vector<uint8_t> msgByHeader(const HeaderWithoutPow& h);

    boost::multiprecision::cpp_int hit(const Header& header);
    ...
    HeaderWithoutPow toHeaderWithoutPow(const Header& header) const {
        return HeaderWithoutPow(header.version, header.parentId, header.transactionsRoot, header.timestamp, header.nBits, header.height);
    }
