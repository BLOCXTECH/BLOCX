...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s BLOCX too low! (Should be %s BLOCX)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s BLOCX too high! (Should be %s BLOCX)" % (str(fee), str(target_fee)))
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "blocx.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "blocx.conf")):
        with open(os.path.join(datadir, "blocx.conf"), 'r', encoding='utf8') as f:
            for line in f:
